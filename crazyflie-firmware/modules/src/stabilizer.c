...
static void stabilizerTask(void* param)
{
  ...
  while(1)
  {
    ...
    if (imu6IsCalibrated())
    {
	  ...
	  //Alaa : This line is commented to remove the pid controller of crazyflie 2.0   
      //controllerGetActuatorOutput(&actuatorRoll, &actuatorPitch, &actuatorYaw);
	  commanderGetRPY(&actuatorRoll, &actuatorPitch, &actuatorYaw)
